...
async function init() {
  try {
    const rodadas = JSON.parse(await fs.readFile("jsonFiles/2004.json"));
...
async function salvaTimes() {
  await fs.writeFile("jsonFiles/times.json", JSON.stringify(times, null, 2));
  await fs.writeFile("public/js/times.js", JSON.stringify(times, null, 2));
}

async function salvaG4() {
  await fs.writeFile("jsonFiles/G4.json", JSON.stringify(times.slice(0, 4), null, 2));
}

async function salvaRebaixados() {
  await fs.writeFile("jsonFiles/rebaixados.json", JSON.stringify(times.slice(20, 25), null, 2));
}